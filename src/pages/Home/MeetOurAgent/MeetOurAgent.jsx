...
const MeetOurAgent = () => {
  ...
  return (
    <section className="bg-white py-16 px-6">
      <h2 className="text-5xl font-extrabold text-center text-black mb-12 tracking-wide">Meet Our Agents</h2>
      <div className="max-w-7xl mx-auto grid gap-10 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {agents.map(agent => (
          <div
            key={agent._id}
            className="bg-gray-50 text-black rounded-xl shadow-lg p-8 flex flex-col items-center text-center transition-transform hover:scale-105 hover:shadow-2xl"
          >
            <img
            ...
            <p className="italic">{agent.specialties ? agent.specialties.join(', ') : 'Specialties not listed'}</p>
          </div>
        ))}
      </div>
    </section>
